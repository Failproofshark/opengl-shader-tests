...
(defparameter *vertex-shader-source* (read-file-into-string (asdf:system-relative-pathname 'opengl-shader-test "texture-vertex-shader.glsl")))
(defparameter *fragment-shader-source* (read-file-into-string (asdf:system-relative-pathname 'opengl-shader-test "texture-fragment-shader.glsl")))
  ...
  (with-init (:everything)
    ...
    (with-window (my-window :title "CL-OpenGL test" :flags '(:shown :opengl))
      (with-gl-context (gl-context my-window)
        ...
        ;;the texture-image is the actual loaded image object
        (let ((texture-image (sdl2-image:load-image (asdf:system-relative-pathname :opengl-shader-test "image.jpg")))
          ...
          (gl:tex-parameter :texture-2d :texture-wrap-s :clamp-to-obrder)
